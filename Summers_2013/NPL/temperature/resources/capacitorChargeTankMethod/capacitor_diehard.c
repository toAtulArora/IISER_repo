...
/*
	...
	DDRC &= ~(1<<5);	//just to confirm//lo is tristate, ready for input
	PORTC &= ~(1<<5);	//	
	//SFIOR (special function I/O register)
	SFIOR |= (1<<ACME);	//to enable multiplexer
	ADMUX |= (1<<MUX2)|(0<<MUX1)|(1<<MUX0);	//ADMUX (A2d Multiplexer)//equivalent of lo here

	ACSR = (1<<ACIC)|(1<<ACIS1)|(0<<ACBG);	//ACIS1 selects falling edge and internal band gap	//ACSR (analog comparator control & state register) 	//Always clear interrupt enable (ACIE) before changing this
	ACSR |= (1<<ACIE);		//enable Comparator Interrupt Enable Now, after selection of right bits

	ICR1=0;			//Input Capture register
	TCNT1=0;		//Timer 1 register

	//TIFR |= (1<<TOV1);	//Input Capture Flag cleared before chagning
	TIMSK |= (1<<TOIE1);	//Enable Interrupt for Input Capture	//Timer Interrupt Mask Register
...
*/	
	...
	//and set interrupts
	DDRC &= ~(1<<lo);	//just to confirm//lo is tristate, ready for input
	PORTC &= ~(1<<lo);	//	
	...
	ICR1 = 0;
	TCNT1 = 0;
	TIFR |= _BV(ICF1);			// clear input capture flag bit	
	...
	//sprintf(test,"LEVEL A %d  ",keepcount);
	//lcd_string(test,1);
	//_delay_ms(100);
	...
	//ACSR &= ~_BV(ACIC); 	// disable AC input capture..timer1 relation
