...
/*
	...
	//Pumping
		...
		DDRD |= _BV(3);		// Drive is output
		
		drive_ctr = drive_length;	//drive counter is set
	//Direct Pumping
		for (drive_ctr = drive_length; --drive_ctr >= 0; ) {
			DDRC &= ~(1 << hi);		//top is tristate
			
			PORTC &= ~(1 << lo); 	//low is ground
			DDRC |= (1<<lo);		//low is on
			
			PORTD |= (1 << 3);		//PD3..for drive +V
			//asm("nop");
			//asm("nop");
			_delay_ms(10);
			
			DDRC &= ~(1 << lo);		//low is tristate
			
			PORTC &= ~(1 << hi); 	//Top is ground
			DDRC |= (1 << hi);		//Enable output on top
			

			PORTD &= ~(1 << 3);		//PD3..for drive ground
			//asm("nop");
			//asm("nop");
			_delay_ms(10);
		}
...
*/
		...
		//top is +V
		PORTC |= (1 << hi); 	//PC4 or hi is +V
		//bottom is ground
		PORTC &= ~(1 << lo); 	//PC5 or lo is 0
		_delay_ms(40);			//some delay although theoretically not required
		//Now lo is tristate
		DDRC &= ~(1 << lo);
